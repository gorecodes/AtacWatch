...
function FavoriteCard({ stop_id, name, code }: { stop_id: string; name: string; code: string | null }) {
  ...
  return (
    <Link
      ...
      className="block rounded-xl border border-neutral-800 bg-neutral-900 px-3 py-2.5 active:bg-neutral-800"
    >
      <div className="mb-2 flex items-baseline gap-1.5">
        <span className="text-sm font-semibold truncate">{name}</span>
        {code && <span className="text-xs text-neutral-600 shrink-0">#{code}</span>}
      </div>
      ...
      {ready && arrivals.length === 0 && (
        <p className="text-xs text-neutral-600">Nessun passaggio nei prossimi 90 min</p>
      )}
          ...
          {arrivals.map((a, i) => (
            <li key={`${a.route_id}-${a.direction_id}-${a.eta_ts}-${i}`}
              ...
              <span className={`shrink-0 text-sm font-semibold tabular-nums ${
                a.is_realtime ? "text-emerald-400" : "text-neutral-300"
              }`}>
...
export default function FavoriteStops() {
  ...
  return (
    <section className="mb-6">
      <h2 className="mb-2 text-sm font-semibold text-neutral-400">Preferiti</h2>
