...
async function tick() {
  ...
  if (result.ok) {
    const { vehicles, tripUpdates, alerts, delayGroups, pushSent } = result.stats;
    console.log(`[worker] ok (${ms}ms) — vehicles:${vehicles} tu:${tripUpdates} alerts:${alerts} delay:${delayGroups} push:${pushSent}`);
  } else {
